...
def model_chart(df, prompt, client):
    """
    Generates a SQL query or visualization code based on a natural language prompt.
    This function interacts with a language model to create a Python visualization 
    code using Plotly, tailored to the provided DataFrame and natural language prompt.
    Args:
        df (pandas.DataFrame): The input DataFrame containing the data to be visualized.
        prompt (str): A natural language question or description of the desired visualization.
        client: The client object used to interact with the language model.
    Returns:
        str: A Python code snippet (as a string) that creates a Plotly visualization 
             based on the input data and prompt. The code is ready for execution.
        fig: The Plotly figure object generated by the code snippet.
    """
...
def model_explain(df, prompt, client, question):
    """
    Generates an explanation of the results of a query based on a natural language prompt.
    This function interacts with a language model to create an explanation of the results
    of a query or DataFrame based on a natural language prompt.
    Args:
        df (dict): The data used in the query or DataFrame.
        prompt (str): A natural language question or description of the query or DataFrame.
        client: The client object used to interact with the language model.
    Returns:
        str: An explanation of the results of the query or DataFrame in a clear and concise manner.
    """
    ...
    Explain the results of the query in a way that is easy to understand for a non-technical audience.
    Don't include the SQL query, just the explanation of the results. Concise and clear explanations are preferred.
    Also, consider the question asked and the data provided.
    """

    chat_completion = client.chat.completions.create(
        messages=[
            {"role": "system", "content": "You are a data analyst who needs to explain the results of a query and dataframe."},
...
def execute_query_chart(df, prompt, client):
    """
    Executes a SQL query and generates a Plotly chart based on the results.
    Args:
        df (pandas.DataFrame): The input DataFrame containing the data to be visualized.
        prompt (str): A natural language question or description of the desired visualization.
        client: The client object used to interact with the language model.
    Returns:
        tuple: (plotly.Figure, str) The Plotly figure object and the Python code used to generate it
    """
...
def process_command(command, option, question):
    """
    ...
    if command == "/explain":
        if option in ["Report Generation", "Chart Generation"]:
            for msg in reversed(st.session_state[f"messages_{option}"]):
                if "prompt" in msg:
                    explanation = model_explain(msg["raw_data"], msg["prompt"], st.session_state.client, question)
                    return {
                        "role": "assistant", 
                        "content": explanation
                    }
...
def main():
    ...
    for msg in st.session_state[f"messages_{option}"]:
        if "raw_data" in msg and "chart" not in msg:
    ...
    if prompt:
        ...
        if prompt.startswith("/"):
            command = prompt.split()[0]
            question = prompt[len(command):].strip()
            command_response = process_command(command, option, question)
